...
const phaseLabel: Record<string, string> = {
  ...
  race_week: "RACE WEEK", recovery: "RECOVERY", post: "COMPLETE", unknown: "—",
};
...
export default function EventScreen() {
  ...
  // form state
  const [type, setType] = useState(EVENT_TYPES[3]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const e = await api<any>("/events/current");
      if (e && e.id) {
        setExisting(e);
        setType(e.event_type); setName(e.event_name); setDateIso(e.event_date);
  ...
  const save = async () => {
    ...
    try {
      await api("/events", {
        method: "POST",
        body: {
          event_type: type, event_name: name, event_date: dateIso,
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: theme.space.lg, paddingBottom: 140 }} keyboardShouldPersistTaps="handled">
          {loading ? <ActivityIndicator color={theme.color.brand} /> : (
            <>
              {existing && (
                <View style={styles.countdownCard} testID="event-countdown">
                  <View style={{ flex: 1 }}>
                    <Text style={styles.countLbl}>{String(existing.event_type || "").toUpperCase()}</Text>
                    <Text style={styles.countName}>{existing.event_name}</Text>
                    <Text style={styles.countDate}>{existing.event_date}</Text>
                  </View>
                  <View style={{ alignItems: "flex-end" }}>
                    <Text style={styles.countBig}>{phase.days_to_race >= 0 ? phase.days_to_race : "—"}</Text>
                    <Text style={styles.countBigLbl}>DAYS</Text>
              ...
              <Sect label="EVENT">
                ...
                <View style={styles.chipsWrap}>
                  {EVENT_TYPES.map((t) => (
                    <Pressable key={t} testID={`ev-type-${t}`} onPress={() => setType(t)} style={[styles.chip, type === t && styles.chipActive]}>
                      <Text style={[styles.chipText, type === t && { color: "#fff" }]}>{t}</Text>
                    </Pressable>
                  ))}
                </View>
                <Field label="NAME"><TextInput testID="ev-name" style={styles.input} value={name} onChangeText={setName} placeholder="London Marathon 2026" placeholderTextColor={theme.color.textDim} /></Field>
                <Field label="DATE (YYYY-MM-DD)"><TextInput testID="ev-date" style={styles.input} value={dateIso} onChangeText={setDateIso} placeholder="2026-04-26" placeholderTextColor={theme.color.textDim} /></Field>
              </Sect>
              ...
              <Sect label="PREFERENCES">
                ...
                <Field label="INJURY HISTORY"><TextInput testID="ev-inj" style={[styles.input, { minHeight: 60 }]} multiline value={injuries} onChangeText={setInjuries} placeholder="Right knee — no downhill running" placeholderTextColor={theme.color.textDim} /></Field>
...
const styles = StyleSheet.create({
  ...
  countdownCard: { flexDirection: "row", padding: theme.space.md, backgroundColor: theme.color.brandTint, borderLeftWidth: 4, borderLeftColor: theme.color.brand, borderRadius: theme.radius.md, marginBottom: theme.space.md },
