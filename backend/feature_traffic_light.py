...
# ---------------------------------------------------------------------------
# Iter189n · Cardio detection mirror of the frontend classifier
# (workoutMode.ts::isCardioExercise). Broadened from the previous
# logging_type-only check so cardio-by-name exercises (Easy Run, Row 5k,
# Zone 2 Bike, …) get their per-exercise duration/distance scaled on
# the green→amber downgrade — not just the workout-level `duration_min`.
# ---------------------------------------------------------------------------
...
_STRENGTH_EXCLUDE_RE = re.compile(
...
def _is_cardio_ex(e: dict) -> bool:
    """Backend mirror of frontend `isCardioExercise` used for green→amber
    scaling. Priority:
      1. Explicit `logging_type` = 'cardio' | 'timer' → cardio.
      2. Explicit `logging_type` = 'weighted' | 'bodyweight' | 'mobility'
         → strength (short-circuit; do NOT let duration_sec flip it).
      3. Non-zero `duration_sec` / `duration_min` on the exercise row.
      4. Name matches the cardio regex AND does NOT match the strength
         exclusion list (walking lunge, barbell row, hip thrust, …).
      5. `reps` string hints at time (`30s`, `5min`, `4:00`, `hold`, …).

    Rules 3–5 all indicate "scale time / distance, not sets / reps" — the
    correct amber behaviour for anything the client logs by clock rather
    than by kg / reps grid.
    """
    if not e:
        return False
    ...
    if lt in ("cardio", "timer"):
        return True
    # Explicit strength-family logging_type wins over any duration hint —
    # a Dumbbell Row with duration_sec=60 (rest hint) is still strength.
    if lt in ("weighted", "bodyweight", "mobility"):
        # …unless the NAME is unambiguously cardio (e.g. someone tagged
        # an Easy Run as "bodyweight"). The name regex below will catch
        # that after we fall through — see the final return.
        name = str(e.get("name") or e.get("exercise_name") or "")
        if _CARDIO_NAME_RE.search(name) and not _STRENGTH_EXCLUDE_RE.search(name):
            return True
        return False
    # Priority 3: explicit per-exercise duration field.
    try:
        if int(e.get("duration_sec") or 0) > 0:
            return True
    except Exception:
        pass
    try:
        if int(e.get("duration_min") or 0) > 0:
            return True
    except Exception:
        pass
    # Priority 4: name regex.
    name = str(e.get("name") or e.get("exercise_name") or "")
    if _CARDIO_NAME_RE.search(name) and not _STRENGTH_EXCLUDE_RE.search(name):
        return True
    # Priority 5: reps string time hint.
    reps = str(e.get("reps") or "")
    if reps and _REPS_TIME_HINT_RE.search(reps):
        return True
    return False
...
# Iter189n · Cardio time embedded in the `reps` field.
_REPS_MIN_RE = re.compile(r"(\d+)(\s*)(min|mins|minute|minutes)\b", re.I)
_REPS_SEC_RE = re.compile(r"(\d+)(\s*)(s|sec|secs|second|seconds)\b", re.I)
_REPS_MMSS_RE = re.compile(r"^(\d+):(\d{2})$")


def _scale_time_in_reps(reps: Any, factor: float) -> Optional[str]:
    """If the `reps` string encodes a duration (e.g. '30 min', '45s',
    '5:00'), return a scaled version; otherwise return None so the
    caller can leave the field untouched.

    Cardio bouts generated by the LLM commonly carry their duration
    in this field when there's no dedicated duration_sec/min column,
    so this is essential for the green→amber cardio scaling contract.
    """
    if reps is None:
        return None
    s = str(reps).strip()
    ...
    # mm:ss (e.g. "5:00", "12:30")
    m = _REPS_MMSS_RE.match(s)
    if m:
        total_sec = int(m.group(1)) * 60 + int(m.group(2))
        scaled = max(60, int(round(total_sec * factor)))
        return f"{scaled // 60}:{scaled % 60:02d}"

    # Minute pattern (preserve whitespace between number and unit).
    m = _REPS_MIN_RE.search(s)
    if m:
        val = int(m.group(1))
        scaled = max(5, int(round(val * factor)))
        return _REPS_MIN_RE.sub(f"{scaled}{m.group(2)}{m.group(3)}", s, count=1)

    # Second pattern (preserve whitespace between number and unit).
    m = _REPS_SEC_RE.search(s)
    if m:
        val = int(m.group(1))
        scaled = max(15, int(round(val * factor)))
        return _REPS_SEC_RE.sub(f"{scaled}{m.group(2)}{m.group(3)}", s, count=1)
...
def _derive_amber(green: dict) -> dict:
    ...
    Iter189n · Cardio detection now mirrors the frontend classifier
    (`_is_cardio_ex`) instead of relying on `logging_type` alone, so
    cardio-by-name / cardio-by-duration exercises (Easy Run, Row 5k,
    Zone 2 Bike, …) get their per-exercise duration and distance
    scaled on the green→amber downgrade — not just the workout-level
    `duration_min`. `logging_type` still survives untouched so the
    client renderer picks the cardio logger, not kg/reps.
    """
    factor = 0.65
    ...
    for e in keep:
        ...
        # Iter189n · Full frontend-mirror cardio detection.
        is_cardio = _is_cardio_ex(e)

        if is_cardio:
            # Cardio: scale explicit duration + distance fields.
            ...
            for dist_key in ("distance_m", "distance_km"):
                if e.get(dist_key):
                    try:
                        ...
                        pass
            # Iter189n · Also scale time embedded in the `reps` string
            # (e.g. "30 min", "20 mins", "45s", "5:00"). This is how
            # LLM-generated cardio bouts most often carry their duration
            # when there's no dedicated duration_sec/min field.
            scaled_reps = _scale_time_in_reps(e.get("reps"), factor)
            if scaled_reps is not None:
                e2["reps"] = scaled_reps
        else:
